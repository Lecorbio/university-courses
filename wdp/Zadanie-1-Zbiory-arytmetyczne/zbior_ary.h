...
//Pojedynczy ciag arytmetyczny przeskalowany wedlug reszty i Q
typedef struct zbior_ary_seg{
	int l, r;
} zbior_ary_seg;

//Wiadro przeskalowanych ciagow arytmetycznych wraz z pomocniczymi zmiennymi
//(ciagi w wiadrze sa posortowane po l)
typedef struct zbior_ary_bucket{
	int rem, len, cap; //reszta modulo Q, liczba ciagow, wielkosc tablicy
	zbior_ary_seg *segs;
} zbior_ary_bucket;

//Tablica wiader przeskalowanych ciagow arytmetycznych wraz z pomocniczymi zmiennymi
//(wiadra sa posortowane po reszcie modulo Q)
typedef struct zbior_ary {
  int n, cap; //liczba tablic, wielkosc tablicy tablic
  zbior_ary_bucket *buckets;
} zbior_ary;
